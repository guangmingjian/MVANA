...
class MVANA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, gcn_nums, dropout, g_name,
        ...
        if self.alpha == 0:
            ...
            self.smus = nn.ModuleList(
                [Attv2(gcn_h_dim, gcn_h_dim, 2, att_drop_ratio, temperature=1.0) for _ in range(sz_c)])
    ...
    def loss(self, y_pre, y_true):
        self.l1 = F.cross_entropy(y_pre, y_true)
        return self.l1 + self.beta * self.div_loss
